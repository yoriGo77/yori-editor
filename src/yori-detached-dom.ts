 ...
 * 先用 DocumentFragment 作临时父级，再把节点挪到 rich 树或别处。
 */
export function yoriDetachedEl<K extends keyof HTMLElementTagNameMap>(tag: K): HTMLElementTagNameMap[K] {
  /* eslint-disable-next-line obsidianmd/prefer-create-el -- 原生 DocumentFragment；不能用 document.createEl（会挂到 document 上）。 */
